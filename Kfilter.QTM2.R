...
Kfilter.QTM2 <-
  function(num,y,A,mu0,Sigma0,Phi,Ups,Gam,cQ,cR,input){
    ...
    # Make sure siginv is not singular
    # if (is.na(sig[,,1])) {
    #   siginv=0
    # }
    # else if (sig[,,1]<=0) { 
    ...
    if (sigmat>0 )
    {
      like = log(sigmat) + t(innov[,,1])%*%siginv%*%innov[,,1]   # -log(likelihood)
    ...
    like=0.5*like
    list(xp=xp,Pp=Pp,xf=xf,Pf=Pf,like=like,innov=innov,sig=sig,Kn=K,Innov=innov,Q=Q,sig=sig)
  }
